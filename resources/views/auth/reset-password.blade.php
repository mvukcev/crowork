<x-guest-layout>
    <div class="cw-surface p-7 md:p-8">
        <p class="cw-kicker mb-2">{{ __('auth.reset_password') }}</p>
        <h1 class="text-2xl font-semibold text-slate-900 mb-6">{{ __('auth.reset_password_intro') }}</h1>
        ...
        <form method="POST" action="{{ route('password.store') }}" class="space-y-4">
            ...
            <div>
                <label class="cw-label" for="password">{{ __('auth.new_password') }}</label>
            ...
            <button type="submit" class="cw-button-primary">{{ __('auth.reset_password') }}</button>
        </form>
    </div>
</x-guest-layout>
