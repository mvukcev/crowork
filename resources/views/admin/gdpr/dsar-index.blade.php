...
<div class="cw-surface min-h-screen">
    <div class="cw-shell-spacing py-8">
        <h1 class="cw-heading-1 mb-4">DSAR Requests</h1>
        ...
        <div class="cw-card-shell p-4 mb-6">
            <h2 class="cw-heading-3 mb-3">Create DSAR Request</h2>
            <form method="POST" action="{{ route('admin.gdpr.dsar.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-3">
                @csrf
                <input class="cw-field" name="requester_user_id" placeholder="Requester user id (optional)">
                <input class="cw-field" name="requester_email" placeholder="Requester email (optional)">
                ...
                <button class="cw-button-primary md:col-span-2" type="submit">Create DSAR request</button>
            </form>
        </div>
                ...
                <tbody>
                    @foreach($requests as $row)
                        <tr class="border-t border-neutral-200">
                            ...
                            <td class="px-3 py-2">{{ $row->due_at?->format('Y-m-d H:i') ?: 'N/A' }}</td>
                            <td class="px-3 py-2">{{ $row->assignedAdmin?->name ?: 'Unassigned' }}</td>
                            <td class="px-3 py-2"><a href="{{ route('admin.gdpr.dsar.show', $row) }}" class="text-blue-700">Open</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
