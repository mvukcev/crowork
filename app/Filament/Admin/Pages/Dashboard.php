...
class Dashboard extends BaseDashboard
{
    ...
    {
        return [
            Action::make('resyncHzzJobs')
                ->label('Resync HZZ Jobs')
                ->icon('heroicon-o-arrow-path')
                ->color('warning')
                ->modalHeading('Resync HZZ Jobs')
                ->modalDescription('Imports fresh HZZ jobs and optionally updates existing HZZ records by source reference.')
                ->form([
                    TextInput::make('url')
                        ->label('HZZ Feed URL')
                        ->url()
